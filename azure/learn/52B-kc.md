...
1. Which is the best way for Tailwind Traders to ensure that the team deploys only _cost-effective_ virtual machine SKU sizes?
...
1. Which is likely the best way for Tailwind Traders to identify which billing department each Azure resource belongs to?

- [ ] Track resource usage in a spreadsheet.
- [ ] _Split the deployment into separate Azure subscriptions_, where each subscription belongs to its own billing department.  
That's incorrect.  
